...
def stopProgram():
    global ifProgramRun
    ifProgramRun = False
    #print("ifProgramRun:", ifProgramRun)
    stopButton.config(state = 'disable')

def savePage(no):
    print("savePage:", no)
    file = open('count.dat', 'w')
    file.writelines(str(no)+'\n')
    file.close()
...
def removeContents():
    ...
    if removeComments.get():
        ...
        while True:
            ...
            link = soup.select('div.list_title > a.list_subject')
            #print(link)
            print("remove page:", list_no)
            textout="댓글을 삭제중입니다. 페이지:"+str(list_no)+"/3426"
            messageLabel.config(text = textout)
            print("삭제할 댓글 갯수:", len(link))
            for l in link:
                if not ifProgramRun:
                    savePage(list_no-1)
                    tk.quit()
...
buttonFrame.configure(background='deepskyblue')
buttonFrame.pack(padx = 10, pady = 10)

removeContentsThread = threading.Thread(target=removeContents)
removeButton = Button(buttonFrame, text = "삭제", state = 'disabled', command = lambda: removeContentsThread.start())
removeButton.grid(row=0, column=0)
stopButton = Button(buttonFrame, text = "나감", state = 'disabled', command = lambda: stopProgram())
